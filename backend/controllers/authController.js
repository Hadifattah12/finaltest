...
const signUp = async (request, reply) => {
  try {
    ...
    console.log('Generated verification token for signup:', emailVerificationToken.substring(0, 10) + '...');
    ...
    // Delay email sending to allow ngrok to start
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        BASE_URL = await getNgrokUrl();
        console.log('✅ Using ngrok URL:', BASE_URL);
        break;
      } catch (error) {
        attempts++;
        console.log(`Ngrok attempt ${attempts}/${maxAttempts} failed:`, error.message);
        if (attempts < maxAttempts) {
          console.log('Waiting 1 second before retry...');
          await new Promise(resolve => setTimeout(resolve, 1000));
        } else {
          console.log('❌ All ngrok attempts failed, using fallback URL');
          BASE_URL = process.env.PUBLIC_URL || 'https://c1r6s6.42beirut.com:3000';
        }
      }
    }
